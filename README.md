...
# Future work
We are planning to work on applying current work on actual protein dataset (Plinder), solve the problem of cropping the binding site of proteins and finding the optimal positions in the 3D space for those proteins. Building a webapp that allows user to upload protein to find potential compliments with out algorithm and display the visualization of those complimentary proteins.
